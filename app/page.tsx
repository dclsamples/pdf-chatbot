	...
	return (
		<main className="flex min-h-screen flex-col items-center p-24">
			<div
				...
				<input {...getInputProps()} />
				<p style={{ color: "#fff" }}>Upload a PDF to add new data</p>
			</div>
			...
			<div className="mx-auto w-full items-center max-w-md py-24 flex flex-col stretch">
				<form onSubmit={handleSubmit} className="flex flex-col gap-4">
					...
					<button disabled={isLoading} type="submit" className="py-2 border rounded-lg bg-gray-900 text-sm px-6">
						Submit
					</button>

					<p className="text-center">Completion result: {completion === "" ? "Thinking..." : completion}</p>
				</form>
			</div>
		</main>
	);
